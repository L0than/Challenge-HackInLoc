...
import { ObjectItem, NavigationContextType } from "../utils/types";

interface DesktopRouteDetailsProps {
  objects: ObjectItem[];
...
function DesktopRouteDetails({
  ...
  navigation,
  setNavigation,
}: DesktopRouteDetailsProps) {
  // Passa os parâmetros para o hook useRouteDetails
  const { object, handleLeave } = useRouteDetails({
    objects,
    navigation,
    setNavigation,
  });
  ...
  return (
    <div className="md:w-fit md:visible invisible hidden md:flex flex-row pl-6">
      <div className="flex flex-inline rounded w-full bg-white navigation-info-pulse">
        ...
        <div className="flex flex-col h-full justify-center px-4"></div>
